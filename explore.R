...
summaryOrientation = trial[!is.na(error), .(or = GetOrientation(stimulus), error, exp = factor(exp), run)]

ggplot(summaryOrientation, aes(x = or, y = error, color = exp)) + geom_point(alpha = 0.2) + geom_smooth() + facet_grid(.~run)

# Hist of stimulus orientation/angle, compare each subject to the average trend (facet_wrap(~subjectId))
